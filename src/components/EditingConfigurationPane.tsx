...
interface EditingConfigurationPaneProps {
...
export interface GenerationConfig {
    ...
    include: string;
    exclude: string;
    numberOfImages: number; // Keep for consistency, but will be 1
}

export function EditingConfigurationPane({ onGenerate, isGenerating, uploadedImage, onImageUpload }: EditingConfigurationPaneProps) {
    ...
    const handleGenerate = () => {
        ...
        onGenerate({
            ...
            numberOfImages: 1, // Single image editing always processes one image
        });
    };
    ...
    return (
        <div className="space-y-6 p-6">
            ...
            <div className="mb-6">
                ...
                <Input type="file" accept="image/*" onChange={handleImageUpload} disabled={isGenerating} className="mb-3" />
                {uploadedImage && (
                    <div className="mt-3">
                        <img
                            src={uploadedImage}
            ...
            <div className="mb-6">
                <Button onClick={handleGenerate} disabled={isGenerating} size="lg" className="w-full">
                    {isGenerating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Editing...</> : 'Edit Image'}
                </Button>
            </div>
            ...
            {uploadedImage && (
                <Dialog open={showImagePreview} onOpenChange={setShowImagePreview}>
                    <DialogContent className="max-w-4xl p-0">
                        <img src={uploadedImage} alt="Uploaded preview" className="w-full h-auto rounded-md" />
                    </DialogContent>
                </Dialog>
            )}
        </div>
